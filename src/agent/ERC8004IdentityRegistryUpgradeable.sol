 ...
 * @dev Upgradeable ERC-8004 Identity Registry using ERC-7201 namespaced storage.
 */
contract ERC8004IdentityRegistryUpgradeable is
    Initializable,
    ERC721URIStorageUpgradeable,
    OwnableUpgradeable,
    EIP712Upgradeable,
    IERC8004IdentityRegistry
{
    ...
    /// @custom:storage-location erc7201:curatedcontracts.storage.ERC8004Identity
    struct ERC8004IdentityStorage {
        uint256 _lastId;
        mapping(uint256 agentId => mapping(string metadataKey => bytes metadataValue)) _metadata;
    }

    // keccak256(abi.encode(uint256(keccak256("curatedcontracts.storage.ERC8004Identity")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant ERC8004IdentityStorageLocation =
        0xa656390e6e730af8a12ceddd6335b5bef46b906823f26f003418b72c13241d00;
    ...
    function _getERC8004IdentityStorage() private pure returns (ERC8004IdentityStorage storage $) {
        assembly {
            $.slot := ERC8004IdentityStorageLocation
        }
    }

    function __ERC8004IdentityRegistry_init() internal onlyInitializing {
    ...
    function getMetadata(uint256 agentId, string calldata metadataKey) external view returns (bytes memory) {
        ...
        return _getERC8004IdentityStorage()._metadata[agentId][metadataKey];
    }
    ...
    function getAgentWallet(uint256 agentId) external view returns (address) {
        _requireOwned(agentId);
        return address(bytes20(_getERC8004IdentityStorage()._metadata[agentId]["agentWallet"]));
    }
    ...
    function setAgentWallet(uint256 agentId, address newWallet, uint256 deadline, bytes calldata signature) external {
        ...
        ERC8004IdentityStorage storage $ = _getERC8004IdentityStorage();
    ...
    function unsetAgentWallet(uint256 agentId) external {
        _checkAuthorized(agentId);
        delete _getERC8004IdentityStorage()._metadata[agentId]["agentWallet"];
    ...
    function _setAgentMetadata(uint256 agentId, string memory metadataKey, bytes memory metadataValue) private {
        ...
        _getERC8004IdentityStorage()._metadata[agentId][metadataKey] = metadataValue;
    ...
    function _update(address to, uint256 tokenId, address auth) internal override returns (address) {
        ...
        if (from != address(0) && to != address(0)) {
            delete _getERC8004IdentityStorage()._metadata[tokenId]["agentWallet"];
